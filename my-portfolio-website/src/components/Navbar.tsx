    ...
    <nav className="w-full flex items-center justify-between mt-8">
        ...
        <ul className="flex items-center justify-center text-gray-400 mr-24 font-medium text-xl  cursor-pointer">
            {['Home', 'About', 'Portfolio', 'Services', 'Experience', 'Contact'].map((item, index) => (
              <li key={index} className="relative mr-14 py-3 after:content-[''] after:h-1 after:w-0 after:bg-emerald-500 after:absolute after:left-0 after:bottom-0 after:transition-all after:duration-600 after:ease-in-out after:transform after:-translate-y-0 hover:after:w-full hover:text-gray-200">
                {item}
              </li>
            ))}
        </ul>
